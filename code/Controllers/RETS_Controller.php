...
*/
		        ...
		        if($params['ID'] == "all") {
			        ...
			        $query = /*"(idx_dt = {$previous_start_time}+),*/"(timestamp_sql = {$previous_start_time}+),(lp_dol = $minVal-$maxVal),(s_r = Sale)";
	...
	public function createMLSListing($MLSRecord, $class, $board) {
		...
		if($MLSListing->CityID != 0){
			$hood = Neighbourhood::get()->where("Name = '".$listingHood."'")->First();
			if($hood){
	...
	public function FixImages() {
		...
		foreach ($MLSListings as $MLSlisting) {
			echo " + Checking ".$MLSlisting->MLS." for Images <br> \n";
			$listing = $MLSlisting->MLS;
			for ($i=1; $i<=9; $i++){
				if(Image::get()->filter("Filename", "assets/Homes/MLS/image-{$listing}-{$i}.jpg")->count()){
					echo " + Found assets/Homes/MLS/image-{$listing}-{$i}.jpg for Images <br> \n";
					$image = Image::get()->filter("Filename", "assets/Homes/MLS/image-{$listing}-{$i}.jpg")->First();
					$image->MLSListingID = $MLSlisting->ID;
					$image->write();
				} else {
					echo " + assets/Homes/MLS/image-{$listing}-{$i}.jpg for Images NOT FOUND <br> \n";
				}
	...
	public function CheckAvail($offset) {
		global $_RETS_SERVER_INFO;
		$rets_login_url = $_RETS_SERVER_INFO['URL'];
		$params = Controller::getURLParams();
		$offset = $params['ID'];
		...
		$listings = MLSListing::get()->limit(50, $offset);
		
		echo " + ".$listings->count()." Listings<br>\n";
		$todaysDate = date("Y-m-d");
		$file_name = "../MLSBackup/AvailCheck-{$todaysDate}.csv";
		$fh = fopen($file_name, "w+");
		fputcsv($fh, array("MLS","Availability"));
		...
		foreach ($listings as $listing) {
			...
			if ($listing->PropType == "Condo") {
				$search = $rets->SearchQuery("Property", "CondoProperty", $query);
			} else {
				$search = $rets->SearchQuery("Property", "ResidentialProperty", $query);
			}
			...
			if ($rets->NumRows($search) > 0) {
				...
				while ($record = $rets->FetchRow($search)) {
                        ...
                        echo "    + Type: ".$record['Style']."<br>\n";
                        if($record['Status'] == "U") {
	                        echo "  + ".$MLSnum." Unavailable<br>\n";
	                        fputcsv($fh, array($MLSnum,"Deleted"));
	                        $listing->delete();
	                        
                        }
                       echo "  + ".$MLSnum." Still Available<br>\n";
                       fputcsv($fh, array($MLSnum,"Active"));
                       $offset++;
                }
				
			} else {
				echo "  + ".$MLSnum." Should Delete<br>\n";
				fputcsv($fh, array($MLSnum,"Deleted"));
				$listing->delete();
			}
			echo "  + ".$MLSnum." Check Complete<br><br><br>\n";
