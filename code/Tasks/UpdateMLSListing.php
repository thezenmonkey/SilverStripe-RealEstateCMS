...
class UpdateMLSListing  extends BuildTask {
    protected $title = 'Update MLS Listings';

    protected $description = 'Get Changes Since Last Sync with MLS';
    ...
    function run($request) {
        ...
        $bridge = Config::inst()->get('RETSUtils', 'RETSBridge');
        $retLimit = Config::inst()->get('RETSUtils', 'MLSLimit');
        $mlsField = Config::inst()->get('RETSUtils', 'MLSField');
        $photoSize = Config::inst()->get('RETSUtils', 'PhotoSize');
        ...
        if($retLimit != 0) {
            $retsparams['Limit'] = $retLimit;
        }


        $previous_start_time = date("Y-m-d", time() - 60 * 60 * 24)."T00:00:00";

        $query = RETSUtils::BuildRetsQuery('Update', $previous_start_time);

        if(!$query) {
            return "FAIL: No Query";
        }

        $rets = RETSUtils::RetsConnectUpdate();
        ...
        foreach ($property_classes as $class) {
                ...
                while ($record = $rets->FetchRow($search)) {
                    ...
                    if(class_exists('CustomMLSFilter')) {
                        if(CustomMLSFilter::ImportFilter($record)) {
                            ...
                            array_push($listingEdited, RETSUtils::CreateMLSListing($record, $class, $bridge, $mlsField));
                        }
                    } else {
                        array_push($listingEdited, RETSUtils::CreateMLSListing($record, $class, $bridge, $mlsField));
                    }
            ...
            echo "  - Get Images<br>\n";
            RETSUtils::MLSImageUpdate($listingEdited, $rets, $photoSize);
